...
export const callApiUserNotification = (payload) =>
  ...
  export const callApiUserMarkAsRead = (id) =>
    ...
    new Promise(async (resolve, reject) => {
      try {
        const url = id ? `http://localhost:8000/user-mark-read/${id}` : `http://localhost:8000/user-mark-read`;
        const response = await axiosConfig({
          method: "put",
          url: url,
        });
    ...
    export const callApiAdminMarkAsRead = (id) =>
        ...
        new Promise(async (resolve, reject) => {
          try {
            const url = id ? `http://localhost:8000/admin-mark-read/${id}` : `http://localhost:8000/admin-mark-read`;
            const response = await axiosConfig({
              method: "put",
              url: url,
            });
